...
/**
 * TimeProfileWindow by Chee Wai Lee updated by Isaac Dooley
 * 
 */
public class TimeProfileWindow extends GenericGraphWindow implements ActionListener, Clickable {
	...
	// Markers that are drawn at certain times (doubles in units of x axis bins) to
	// identify phases or iterations
	...
	private void createLayout() {
		...
		showMarkersCheckBox.setToolTipText(
				"Draw vertical lines at time associated with any user supplied notes containing\"***\"?");
		...
		hideMouseoversCheckBox.setToolTipText(
				"Disable the displaying of information associated with the data under the mouse pointer.");
	...
	private void generateLegend(boolean useShortenedNames) {
		...
		for (int ep = 0; ep < numEPs; ep++) {
			if (useShortenedNames)
				l.add(new SortableEPs(sums[ep], MainWindow.runObject[myRun].getPrettyEntryNameByIndex(ep),
						MainWindow.runObject[myRun].getEPColorMap()[ep]));
			else
				l.add(new SortableEPs(sums[ep], MainWindow.runObject[myRun].getEntryNameByIndex(ep),
						MainWindow.runObject[myRun].getEPColorMap()[ep]));
	...
	public void showDialog() {
		...
		if (!dialog.isCancelled()) {
			...
			if (MainWindow.runObject[myRun].hasLogFiles()) {
				...
				startInterval = (int) startTime / (int) intervalSize;
				endInterval = ((int) endTime / (int) intervalSize) - 1;
			}

			System.out.println("Props: intervalSize:" + intervalSize + "- startInterval:" + startInterval
					+ "- endInterval:" + endInterval + "- startTime:" + startTime);
			...
			final SwingWorker worker = new SwingWorker() {
				public Object doInBackground() {
					...
					if (MainWindow.runObject[myRun].hasLogFiles()) { // Bilge
						...
						double[][][] graphDataAccumulators = new double[numResultAccumulators][numIntervals][numEPs
								+ special];

						int pIdx = 0;
						for (Integer pe : processorList) {
							readyReaders.add(new ThreadedFileReader(pe, intervalSize, myRun, startInterval, endInterval,
									phaseMarkers, graphDataAccumulators[pIdx % numResultAccumulators]));
							pIdx++;
						}
						...
						} else if (MainWindow.runObject[myRun].guiRoot != null
								&& MainWindow.runObject[myRun].guiRoot.isVisible()) {
							guiRootForProgressBar = MainWindow.runObject[myRun].guiRoot;
						}

						// Pass this list of threads to a class that manages/runs the threads nicely
						TimedProgressThreadExecutor threadManager = new TimedProgressThreadExecutor(
								"Loading Time Profile in Parallel", readyReaders, guiRootForProgressBar, true);
					...
					} else if (MainWindow.runObject[myRun].hasSumDetailFiles()) // Bilge
					{
						...
						// System.out.println("hasSumDetailFiles - LOAD DATA. numIntervals: " +
						// numIntervals);

						SortedSet<Integer> availablePEs = MainWindow.runObject[myRun]
								.getValidProcessorList(ProjMain.SUMDETAIL);
						...
						// data
						int[][] idleTemp = MainWindow.runObject[myRun].sumAnalyzer.getSystemUsageData(startInterval,
						...
						int[][] temp = MainWindow.runObject[myRun].sumAnalyzer.getSystemUsageData(startInterval,
								endInterval, intervalSize);
						...
						for (int peIdx = 0; peIdx < numProcessors; peIdx++) {
							...
							for (int interval = 0; interval < numIntervals; interval++) {
								if (idleData[peIdx] != null && idleData[peIdx].length > interval) {
									...
									graphData[interval][numEPs] -= idleData[peIdx][interval] * 0.01 * intervalSize; // overhead
																													// =
																													// -
																													// idle
																													// time
					...
					for (int interval = 0; interval < graphData.length; interval++) {
						for (int e = 0; e < graphData[interval].length; e++) {
							graphData[interval][e] = graphData[interval][e] * 100.0
									/ ((double) intervalSize * (double) numProcessors);
						}
					}
					// Bilge
					...
					for (int interval = 0; interval < graphData.length; interval++) {
						...
						if (!valid) {
							System.err.println("Time Profile found bad data for interval " + interval
									+ ". The data for bad intervals will be zero-ed out. This problem is either a log file corruption issue, or a bug in Projections.");
							for (int e = 0; e < graphData[interval].length; e++) {
	...
	public String[] getPopup(int xVal, int yVal) {
		...
		rString[0] = "Time Interval: " + U.humanReadableString((xVal + startInterval) * intervalSize) + " to "
	...
	public void toolClickResponse(MouseEvent e, int xVal, int yVal) {
		...
		if (displaySlopes) {
			...
			JPanelToImage.saveToFileChooserSelection(graphCanvas, "Save Screenshot Image",
					"./TimeProfileScreenshot.png");
		}
