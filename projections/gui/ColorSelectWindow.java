   ...
   implements AdjustmentListener, ActionListener
{
   ...
   private int w, h;
   private Panel p1, p2;                    
   private ColorWindowFrame parent;
   ...
   public void actionPerformed(ActionEvent evt)
   {
      ...
      if(b == bOK)
      {
         parent.applyNewColor(currentColor);
      }   
      Close(); 
   }
   ...
   public void paint(Graphics g)
   {
      ...
      g.drawRect(p1.getBounds().x-1, p1.getBounds().y-1, p1.getBounds().width+1, p1.getBounds().height+1);
      g.drawRect(p2.getBounds().x-1, p2.getBounds().y-1, p2.getBounds().width+1, p2.getBounds().height+1);  
