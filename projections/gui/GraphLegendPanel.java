   ...
   implements ActionListener
{
   ...
   private class LegendCanvas extends Canvas
   {
	  ...
	  public void print(Graphics pg)
	  {
		if(data == null)
		 ...
		 {
			if((data.ymode == GraphData.MSGS && data.onGraph[a].ymode != GraphData.TIME) ||
			   (data.ymode == GraphData.TIME && data.onGraph[a].ymode != GraphData.MSGS))
			{  
			
			   pg.setColor(data.onGraph[a].color);
			   s = data.onGraph[a].name;
			   if(!(data.onGraph[a].type.equals("%") || data.onGraph[a].type.equals("Msgs")))
	  ...
	  {    
		 ...
		 for(int a=0; a<data.onGraph.length; a++)
		 {
			...
			{   
			   ...
			   height += textheight;
			   if(fm.stringWidth(s) > width) width = fm.stringWidth(s);
			}
		 }              
		
		
		 if(sp.getViewportSize().width > width) width = sp.getViewportSize().width;
		 if(sp.getViewportSize().height> height) height = sp.getViewportSize().height;
   ...
   public Dimension getMinimumSize() {return new Dimension(80,100);}   
   public Dimension getPreferredSize() {return new Dimension(140,200);}   
   public void paint(Graphics g)
   {
