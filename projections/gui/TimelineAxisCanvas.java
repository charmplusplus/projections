...
public class TimelineAxisCanvas extends Canvas
{
	...
	return (x-beginCoord_)/(double)(endCoord_-beginCoord_)*
	...
	if (data.tlw > 0 && data.ath > 0) {
	    ...
	    if ((type.equals("top") && data.numPs > 1) || 
		(type.equals("bot") && data.numPs > 0)) { 
		long labeloffset = data.timeIncrement - 
		    (data.beginTime % data.timeIncrement);
		if (labeloffset == data.timeIncrement) {
		    labeloffset = 0;
		}
