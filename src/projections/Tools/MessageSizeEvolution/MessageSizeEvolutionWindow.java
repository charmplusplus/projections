...
public class MessageSizeEvolutionWindow
            ...
            final SwingWorker worker = new SwingWorker() {
                ...
                protected Object doInBackground() throws Exception {
                    ...
                    if(!msgLogScale && (timeBinSize == 0 || msgBinSize == 0)) {
                        ...
                        return null;
                    } else if(msgLogScale && msgMinBinSize < 128) {
                        JOptionPane.showMessageDialog(null, "You cannot enter a starting bin size less than 128.", "Error", JOptionPane.ERROR_MESSAGE);
                        System.out.println("You cannot enter a starting bin size less than 128.");
                        return null;
                    }
                    numEPs = MainWindow.runObject[myRun].getNumUserEntries() + 1;
                    counts = new double[timeNumBins + 1][msgNumBins + 1][numEPs];

                    // Create a list of worker threads
                    LinkedList<Runnable> readyReaders = new LinkedList<Runnable>();
                    for(Integer nextPe : dialog.getSelectedProcessors()) {
                        readyReaders.add( new ThreadedFileReader(counts, nextPe, dialog.getStartTime(), dialog.getEndTime(), timeNumBins, timeBinSize, timeMinBinSize, msgNumBins, msgBinSize, msgMinBinSize, msgLogScale, msgCreationEvent));
                    }
    ...
    private void createPlot() {
        ...
        double[][] heatMap = new double[counts.length][];
        double maxVal = Double.MIN_VALUE, minVal = Double.MAX_VALUE;
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        for(int i = 0; i < counts.length; i++) {
            heatMap[i] = new double[counts[i].length];
            for(int j = counts[i].length - 1; j >= 0; j--) {
                ...
                dataset.addValue(timeBinSize,
                        U.humanReadableString((i * timeBinSize) + timeMinBinSize),
                        msgKey
                );
        ...
        renderer.setBaseToolTipGenerator(new CustomToolTipGenerator(heatMap, timeNumBins, timeBinSize, timeMinBinSize, msgNumBins, msgBinSize, msgMinBinSize, msgLogScale));
        plot.setRenderer(renderer);

        CustomRangeAxis rangeAxis = new CustomRangeAxis(timeMinBinSize);
    ...
    protected void createMenus() {
        ...
        mSaveScreenshot = new JMenuItem("Save Profile Chart to JPG or PNG");
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(c == mClose)
            ...
            JPanelToImage.saveToFileChooserSelection(chart.createBufferedImage(1100, 700), "Save Evolution Chart", "./MessageSizeEvolution.png");
    }
}
