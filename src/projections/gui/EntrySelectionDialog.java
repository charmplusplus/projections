...
/** 
 * ??????????????????????????????????????????
 * This is a class that should be rewritten in SWING. I'm not sure exactly what its purpose was, 
 * or why it is such a complicated thing. 
 * It appears to be used for choosing colors of entry methods, and enabling/disabling them? 
 *  
 *  References to this class have been commented out in other classes
 * 
 *  - Isaac 2009
 * 
 * */
...
// */
//
public class EntrySelectionDialog extends JFrame 
...
//			for (int ep=0; ep<numEPs; ep++) {
//				if (existsArray[ep]) {
//					entryPointSelection[type][ep].setState(stateArray[ep]);
//					entryPointColors[type][ep].setColor(colorArray[ep]);
//				}
//			}
//		} 
//		dispose();
